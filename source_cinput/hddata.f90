...
subroutine EvaluateHd0(igeom, hmat, dhmat)
  ...
  implicit none
  double precision, intent(in) :: igeom(ncoord)
  double precision, intent(out):: hmat(nstates,nstates)
  double precision, intent(out):: dhmat(ncoord,nstates,nstates)
  double precision, allocatable :: dhtmp(:,:,:),dtmp(:)
  double precision :: oop(noopc)
  integer :: R
  integer :: i, j, k, o
  integer :: nrn
